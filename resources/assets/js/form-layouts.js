...
(function () {
  const intlTelInputs = document.querySelectorAll("[intlTelInput]");
  const intlTelErrorMap = ["Invalid number", "Invalid country code", "Too short", "Too long", "Invalid number"];
  intlTelInputs.forEach(function (intlTelInput) {
    intlTelInput.iti = window.intlTelInput(intlTelInput, {
      hiddenInput: intlTelInput.getAttribute('hiddenInput'),
      showFlags: false,
      onlyCountries: ['eg', 'ae', 'sa'],
      initialCountry: "eg",
      utilsScript: '/assets/vendor/libs/intlTelInput/utils.js',
    });

    intlTelInput.onchange = () => {
      if (!intlTelInput.iti.isValidNumber()) {
        const errorCode = intlTelInput.iti.getValidationError();
        const errorMsg = intlTelErrorMap[errorCode <= 0 ? 0 : errorCode];
          intlTelInput.setCustomValidity(errorMsg);
      }
      else
      {
        intlTelInput.setCustomValidity('');
      }
    };
  })
